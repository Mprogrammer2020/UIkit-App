//
//  SeeAllRecentlyDele+Datas.swift
//  Semilla
//
...
class SeeAllRecentlyColVwDatasouces: NSObject,UICollectionViewDataSource {
    
    //MARK: Variable Declaration
    private let viewModel: SeeAllRecentlyItemsVM!
    private let seeAllRecentlyColVwProperties: SeeAllRecentlyItemsProperties!
    var callBackToNavigatePopup:((Int)->())?
    
    //MARK: Intialization
    init (viewModel:SeeAllRecentlyItemsVM, seeAllRecentlyColVwProperties:SeeAllRecentlyItemsProperties) {
        self.viewModel = viewModel
        self.seeAllRecentlyColVwProperties = seeAllRecentlyColVwProperties
    }
    
    //MARK: Collectionview Datasource Method
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if collectionView == seeAllRecentlyColVwProperties.colVwCategeories {
            return viewModel.arrCategoryList.count
        } else {
            if viewModel.arrRecentList.count == 0 {
                seeAllRecentlyColVwProperties.vwNoItemFound.isHidden = false
                collectionView.setEmptyMessage(AlertMessages.noItemsFound)
            } else {
                seeAllRecentlyColVwProperties.vwNoItemFound.isHidden = true
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == seeAllRecentlyColVwProperties.colVwCategeories {
            ...
            if data.id == viewModel.selectCategoryId {
            ...
            cell.lblCostPerGram.text = "$\(CommonMethod.shared.convertToColombianPeso(amount: (data.pricePerUnit ?? 0)) ?? "")" // "$\((data.pricePerUnit ?? 0).addZero)/\(data.unit ?? "")"
    ...
    @objc func btnAddAction (_ sender:UIButton) {
        ...
        if data.quantity == 0 {
            Alerts.shared.showAlert(title: AlertMessages.appName, message: "Out of stock")
        } else {
            if sender.tag < viewModel.arrRecentList.count {
                if let cell = seeAllRecentlyColVwProperties.colVwRecentlyItems.cellForItem(at: IndexPath(item: index, section: 0)) as? RecentlyListedCVC {
...
//                        cell.btnIncreaceItem.isUserInteractionEnabled = false
                    ...
                    currentController.showToast(message: "Maximum quantity limit reached.", font: appFont(name: AppStyle.Fonts.interMedium, size: 13), textColor: AppStyle.AppColors.appColorGreen)
                }
            }
        }
    }
...
class SeeAllRecentlyColVwDelegate: NSObject,UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    
    //MARK: Variable Declaration
    private let objViewModel: SeeAllRecentlyItemsVM!
    private let seeAllRecentlyColVwProperties: SeeAllRecentlyItemsProperties!
    var callBackToNavigateItemDetails:((Int)->())?
    
    // MARK: Intialization
    init (viewModel:SeeAllRecentlyItemsVM,properties:SeeAllRecentlyItemsProperties) {
        self.objViewModel = viewModel
        self.seeAllRecentlyColVwProperties = properties
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if collectionView == seeAllRecentlyColVwProperties.colVwCategeories {
            ...
            if indexPath.item == 0 {
                return CGSize(width: label.intrinsicContentSize.width + 43, height: collectionView.frame.size.height)
            } else {
                return CGSize(width: label.intrinsicContentSize.width + 30, height: collectionView.frame.size.height)
            }
        } else {
            return CGSize(width: collectionView.frame.size.width/2 - 5, height: 225)
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == seeAllRecentlyColVwProperties.colVwCategeories {
            ...
            if objViewModel.selectCategoryId != (data.id ?? 0) {
                objViewModel.selectCategoryId = data.id ?? 0
                seeAllRecentlyColVwProperties.colVwCategeories.reloadData()
                objViewModel.pageNumber = 0
                objViewModel.recentlyListedApiMethod(data.id ?? 0, isLoader: true) {
                    self.seeAllRecentlyColVwProperties.colVwRecentlyItems.reloadData()
                }
            }
        } else if collectionView == seeAllRecentlyColVwProperties.colVwRecentlyItems {
            self.callBackToNavigateItemDetails?(indexPath.item)
        }
    }
    
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        if scrollView == seeAllRecentlyColVwProperties.colVwRecentlyItems {
            if ((self.seeAllRecentlyColVwProperties.colVwRecentlyItems.contentOffset.y + self.seeAllRecentlyColVwProperties.colVwRecentlyItems.frame.size.height) >= self.seeAllRecentlyColVwProperties.colVwRecentlyItems.contentSize.height) {
                if (self.objViewModel.pageNumber + 1) < (self.objViewModel.totalPages) {
                    ...
                    seeAllRecentlyColVwProperties.activityIndicator.isHidden = false
                    seeAllRecentlyColVwProperties.activityIndicator.startAnimating()
                    seeAllRecentlyColVwProperties.cnstBottomColVwRecentlyListed.constant = 80
                    self.objViewModel.recentlyListedApiMethod(objViewModel.selectCategoryId, isLoader: false) {
                        self.seeAllRecentlyColVwProperties.activityIndicator.isHidden = true
                        self.seeAllRecentlyColVwProperties.activityIndicator.stopAnimating()
                        self.seeAllRecentlyColVwProperties.cnstBottomColVwRecentlyListed.constant = 0
                        self.seeAllRecentlyColVwProperties.colVwRecentlyItems.reloadData()
                    }
                }
            }
